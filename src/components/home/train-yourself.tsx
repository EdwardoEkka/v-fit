    ...
    <section className="w-full py-12 train-yourself text-white relative overflow-hidden">
      <div className="container flex flex-col md:flex-row lg:gap-20 gap-12 justify-center items-center">
        <div className="grid grid-cols-2 grid-rows-2 relative z-10 w-full md:w-1/2 lg:w-1/3 h-[300px] xs:h-[450px] gap-4">
          {images.map((image, index) => (
            <div
              ...
              } ${index == 1 ? "flex flex-col justify-end" : ""} ${
                index == 2 ? "flex flex-col justify-start" : ""
              } ${index == 3 ? "flex justify-start" : ""} `}
            >
              <div
                className={`${index == 0 ? "w-3/4 p-2 t-img-0" : ""} ${
                  index == 1 ? "h-2/3 relative bottom-3 t-img-1" : ""
                } ${index == 2 ? "h-2/3 relative top-3 t-img-2" : ""} ${
                  index == 3 ? "w-3/4 t-img-3" : ""
                } border border-white rounded-xl bg-white`}
              >
                <img
                  src={image}
                  alt={`Fitness ${index}`}
        ...
        <div className="w-full md:w-1/2 lg:w-2/3 px-0 sm:px-4 flex flex-col gap-4">
          ...
          <h1 className="text-3xl xs:text-5xl md:text-3xl lg:text-6xl font-black ">
            TAKE YOUR
          </h1>
          <h1 className="text-3xl xs:text-5xl md:text-3xl lg:text-6xl font-black ">
            TRAINING TO THE
